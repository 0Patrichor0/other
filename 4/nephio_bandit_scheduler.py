...
DEFAULT_SERVICE_PROFILES = {
    "upf": {
        ...
        "required_capabilities": ["sriov", "hugepages", "multus"],
    },
    "amf": {
        "max_rtt_ms": 50,
        "max_jitter_ms": 10,
...
def render_promql(template: str, cluster: str) -> str:
    return template.format(cluster=cluster)
...
def hard_filter(
    ...
    cpu_reserve = cpu_alloc * float(cfg.safety["cpu_ratio"])
    cpu_reserve_cap = float(cfg.safety.get("cpu_reserve_max_cores", cpu_reserve))
    cpu_need = req_cpu + min(cpu_reserve, cpu_reserve_cap)

    mem_reserve = mem_alloc * float(cfg.safety["mem_ratio"])
    mem_reserve_cap = float(cfg.safety.get("mem_reserve_max_gib", mem_alloc / (1024 ** 3))) * 1024 ** 3
    mem_need = req_mem + min(mem_reserve, mem_reserve_cap)
